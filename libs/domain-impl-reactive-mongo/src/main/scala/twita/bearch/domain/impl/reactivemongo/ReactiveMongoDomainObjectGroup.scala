  ...
  extends ObjectDescriptor[EventId, A, D](context) with DomainObjectGroup[EventId, A]
{
  ...
  /**
    * Uses an arbitrary user constraint to build up a Mongo query, and runs that query against this Ojbect Group.
    * @param userConstraint An arbitrary Mongo query (in the form of a JSON object) to be used to constrain the
    *                       result.
    *
    * @return Eventually, Some(A) if is found, or None if no objects meet the criteria.  Note also that this method
    *         will do an additional check if it doesn't find any items that meet the contraint.  The provided
    *         {{userConstraint}} will be combined with the {{listConstraint}} for this group and that query will
    *         be used to find the result.  If none is found then we try another query without the additional
    *         {{listConstraint}}.  If that query returns Some, it means that there is an object in the system that
    *         meets the {{userConstraint}} but that the addition of the {{listConstraint}} excluded that object
    *         from this Object Group{{listConstraint}}.  If that query returns Some, it means that there is an object
    *         in the system that meets the {{userConstraint}} but that the addition of the {{listConstraint}}
    *         excluded that object from this Object Group.  If this is the case, we fail the Future with an
  ...
  protected def getByJsonCrit(userConstraint: JsObject): Future[Option[A]] = {
    getByJsonCritNoListConstraint(userConstraint ++ listConstraint).flatMap {
      ...
      case None => getByJsonCritNoListConstraint(userConstraint).flatMap {
        case Some(s) => Future.failed(new ObjectDoesNotMeetConstraints(userConstraint, listConstraint))
  ...
  /**
    ...
    * @return Eventually, an optional A if is found, or None if no objects meet the criteria
    */
  protected def getByJsonCritNoListConstraint(constraint: JsObject): Future[Option[A]] = {
    for {
      coll <- objCollectionFt
      docOpt <- coll.find(constraint ++ notDeletedConstraint, projection = Some(Json.obj())).one[D]
  ...
  /**
    ...
    *         Domain Object Group.
    */
  protected def getListByJsonCrit(
      constraint: JsObject
